...
extEEPROM eep(kbits_16, 1, 16, 0x50);         //device size, number of devices, page size
...
void setup(void)
{
	...
	Serial << F("dumping...") << endl;
    //dump(0, 2048);
	Serial << _HEX(eep.read(0x3A)) << endl;
	Serial << _HEX(eep.read(0x3B)) << endl;
	Serial << _HEX(eep.read(0x3C)) << endl;
